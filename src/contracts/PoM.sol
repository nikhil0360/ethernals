...
contract Domains is ERC721URIStorage {
    ...
    constructor()
        payable
        ERC721("Proof of Membership", "PoM")
    {
        owner = payable(msg.sender);
    }

    // A register function that adds their names to our mapping
    function createToken(string calldata text) public payable {
        // Combine the name passed into the function  with the TLD
        string memory _text = string(abi.encodePacked(text));
        // Create the SVG (image) for the NFT with the name
        string memory finalSvg = string(
            abi.encodePacked(svgPartOne, _text, svgPartTwo)
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        _text,
                        '", "description": "An NFT to prove that the owner has used the platform", "image": "data:image/svg+xml;base64, "type":"membership"',
